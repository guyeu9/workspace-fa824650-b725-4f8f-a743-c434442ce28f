      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 break-words">JSON 游戏文件验证器</h1>
                <p className="text-slate-500 mt-1 text-sm sm:text-base break-words">验证、修复和编辑你的游戏数据文件</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-start sm:justify-end">
              <button
                ...
                className="px-3 sm:px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-all duration-300 flex items-center gap-2 text-sm min-h-[44px] justify-center"
              >
              ...
              <button
                ...
                className="px-3 sm:px-4 py-2 bg-slate-500 text-white rounded-lg hover:bg-slate-600 transition-all duration-300 flex items-center gap-2 text-sm min-h-[44px] justify-center"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              ...
              <textarea
                ...
                className="w-full h-80 sm:h-96 p-3 sm:p-4 border-2 border-slate-200 rounded-lg font-mono text-xs sm:text-sm focus:border-indigo-500 focus:outline-none resize-none"
                placeholder="在此粘贴或输入JSON内容..."
              />
              <div className="flex flex-wrap gap-2 mt-3">
                <button
                  onClick={validateJson}
                  className="px-3 sm:px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-all duration-300 flex items-center gap-2 text-sm min-w-[44px] min-h-[44px] justify-center flex-1 sm:flex-shrink-0"
                >
                ...
                <button
                  onClick={autoFix}
                  className="px-3 sm:px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-all duration-300 flex items-center gap-2 text-sm min-w-[44px] min-h-[44px] justify-center flex-1 sm:flex-shrink-0"
                >
                  <span>自动修复</span>
                </button>
                <button
                  onClick={exportFixedJson}
                  className="px-3 sm:px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-300 flex items-center gap-2 text-sm min-h-[44px] justify-center flex-1 sm:flex-shrink-0"
                >
            ...
            <div>
              ...
              <div className="h-80 sm:h-96 overflow-y-auto border-2 border-slate-200 rounded-lg p-3 sm:p-4 bg-slate-50">
                {validationResult ? (
                  <div className="space-y-3">
                    <div className={`p-3 rounded-lg ${validationResult.isValid ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-semibold ${validationResult.isValid ? 'text-green-800' : 'text-red-800'} text-sm sm:text-base`}>
                    ...
                    {validationResult.stats && (
                      <div className="grid grid-cols-3 gap-3">
                        <div className="bg-white p-3 rounded-lg border border-slate-200">
                          <div className="text-xl font-bold text-indigo-600">{validationResult.stats.branches}</div>
                          <div className="text-xs sm:text-sm text-slate-500">分支数量</div>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-slate-200">
                          <div className="text-xl font-bold text-emerald-600">{validationResult.stats.choices}</div>
                          <div className="text-xs sm:text-sm text-slate-500">选择数量</div>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-slate-200">
                          <div className="text-xl font-bold text-purple-600">{validationResult.stats.chapters}</div>
                          <div className="text-xs sm:text-sm text-slate-500">章节数量</div>
                        </div>
                      </div>
                    )}
                    ...
                    {validationResult.errors.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-red-800 mb-2 text-sm sm:text-base">错误 ({validationResult.errors.length})</h3>
                            ...
                            <div key={index} className="bg-red-50 border border-red-200 rounded-lg p-2 sm:p-3">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {error.location && (
                                    <div className="text-xs sm:text-sm text-red-600 mt-1 truncate">{error.location}</div>
                                  )}
                                </div>
                              </div>
                            </div>
                    ...
                    {validationResult.warnings.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-yellow-800 mb-2 text-sm sm:text-base">警告 ({validationResult.warnings.length})</h3>
                        <div className="space-y-2">
                          {validationResult.warnings.map((warning, index) => (
                            <div key={index} className="bg-yellow-50 border border-yellow-200 rounded-lg p-2 sm:p-3">
                              <div className="flex items-start gap-2">
                                <span className="text-yellow-500 mt-0.5">⚠️</span>
                                <div className="flex-1 min-w-0">
                                  <div className="text-yellow-800 text-sm break-words">{warning.message}</div>
                                  {warning.location && (
                                    <div className="text-xs sm:text-sm text-yellow-600 mt-1 truncate">{warning.location}</div>
                                  )}
                                </div>
                              </div>
                            </div>
                    ...
                    {validationResult.info.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-blue-800 mb-2 text-sm sm:text-base">信息 ({validationResult.info.length})</h3>
                        <div className="space-y-2">
                          {validationResult.info.map((info, index) => (
                            <div key={index} className="bg-blue-50 border border-blue-200 rounded-lg p-2 sm:p-3">
                              <div className="flex items-start gap-2">
                                <span className="text-blue-500 mt-0.5">ℹ️</span>
                                <div className="flex-1 text-blue-800 text-sm break-words">{info.message}</div>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="text-center text-slate-400 py-12">
                    <IconBox className="w-12 h-12 mx-auto mb-3 opacity-50" />
                    <p className="text-sm sm:text-base">点击"验证JSON"开始验证</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {parsedData && parsedData.branches && (
            <div className="mt-6">
              <h2 className="text-lg sm:text-xl font-bold text-slate-800 mb-3">分支可视化编辑器</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {parsedData.branches.map((branch: any, index: number) => (
                  <div
                    key={branch.branch_id || index}
                    className="bg-white border-2 border-slate-200 rounded-lg p-3 hover:border-indigo-500 transition-all duration-300 cursor-pointer min-w-0"
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-slate-800 mb-1 truncate">{branch.branch_id || '未命名'}</h3>
                    <p className="text-sm text-slate-600 line-clamp-2">{branch.chapter || '无章节'}</p>
                    <p className="text-xs text-slate-400 mt-2 line-clamp-2">{branch.scene_detail?.substring(0, 80) || '无描述'}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-white border-b border-slate-200 p-4 flex items-center justify-between">
                  <h2 className="text-lg sm:text-xl font-bold text-slate-800 truncate">编辑分支: {editingBranch.branch_id}</h2>
                  <button
                    onClick={() => {
                      setShowEditor(false)
                      setSelectedBranch(null)
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full h-24 sm:h-32 px-3 py-2 border border-slate-300 rounded-lg focus:border-indigo-500 focus:outline-none resize-vertical text-sm"
                    />
                  </div>
                  <div>
                        ...
                        <div key={index} className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                          ...
                          <div className="mt-2">
                            ...
                            <textarea
                              ...
                              className="w-full h-12 sm:h-16 px-3 py-2 border border-slate-300 rounded-lg focus:border-indigo-500 focus:outline-none resize-vertical text-sm"
                            />
                          </div>
                        </div>
                      ))}
                ...
                <div className="sticky bottom-0 bg-white border-t border-slate-200 p-4 flex flex-wrap gap-2 justify-end">
                  <button
                    onClick={() => {
                      setShowEditor(false)
                      setSelectedBranch(null)
                    ...
                    className="px-4 py-2 bg-slate-500 text-white rounded-lg hover:bg-slate-600 transition-all duration-300 text-sm min-w-[80px] min-h-[44px] flex items-center justify-center"
                  >
                    取消
                  </button>
                  <button
                    onClick={() => saveBranch()}
                    className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-all duration-300 text-sm min-w-[80px] min-h-[44px] flex items-center justify-center"
                  >
                    保存更改
                  </button>
                </div>
