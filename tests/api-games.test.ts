...
import { GET, POST } from '../src/app/api/games/route';
...
describe('游戏API测试', () => {
  ...
  describe('GET /api/games - 获取游戏列表', () => {
    it('应该成功返回游戏列表', async () => {
      ...
      vi.mocked(db.vote.groupBy).mockResolvedValue(mockVoteStats);
    ...
    it('应该按创建时间降序返回游戏', async () => {
      ...
      vi.mocked(db.game.findMany).mockResolvedValue(mockGames);
      vi.mocked(db.vote.groupBy).mockResolvedValue([]);
    ...
    it('应该处理数据库错误', async () => {
      vi.mocked(db.game.findMany).mockRejectedValue(new Error('数据库连接失败'));
  ...
  describe('POST /api/games - 创建游戏', () => {
    ...
    it('应该处理数据库创建错误', async () => {
      ...
      vi.mocked(db.user.findUnique).mockResolvedValue(mockUser);
      vi.mocked(db.game.create).mockRejectedValue(new Error('数据库错误'));
