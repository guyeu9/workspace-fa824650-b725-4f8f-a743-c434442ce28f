        ...
        if (childBranch.choices && childBranch.choices.length > 0) {
          choice.sub_choices = childBranch.choices.map(c => {
            ...
            if (subChoice.next_branch) {
              subChoice.target_branch_id = subChoice.next_branch;
              delete subChoice.next_branch;
            }
            subChoice.end_game = subChoice.target_branch_id === 'final_hetero' ? true : false;
            return subChoice;
          });
        }
...
function updateNextBranches(branches) {
  ...
  branches.forEach(branch => {
    if (branch.choices) {
      branch.choices.forEach(choice => {
        if (choice.target_branch_id && choice.target_branch_id !== '') {
          if (!branchIds.has(choice.target_branch_id)) {
            invalidRefs.push({
              branch: branch.branch_id,
              choice: choice.id,
              target: choice.target_branch_id
            });
          }
        }
        if (choice.sub_choices) {
          choice.sub_choices.forEach(subChoice => {
            if (subChoice.target_branch_id && subChoice.target_branch_id !== '') {
              if (!branchIds.has(subChoice.target_branch_id)) {
                invalidRefs.push({
                  branch: branch.branch_id,
                  choice: `${choice.id} -> ${subChoice.id}`,
                  target: subChoice.target_branch_id
                });
              }
            }
          });
...
function processBranches(branches) {
  ...
  mergedBranches.forEach(branch => {
    ...
    if (branch.choices) {
      branch.choices.forEach(choice => {
        if (choice.next_branch) {
          choice.target_branch_id = choice.next_branch;
        ...
        if (!choice.end_game) {
          choice.end_game = choice.target_branch_id === 'final_hetero' ? true : false;
        }
        if (choice.sub_choices) {
          choice.sub_choices.forEach(subChoice => {
            if (subChoice.next_branch) {
